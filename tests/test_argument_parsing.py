...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["--include-dependencies", "--start-with=qemu", "run-riscv64-purecap"],
            ["qemu", "disk-image-riscv64-purecap", "run-riscv64-purecap"],
            id="run-start-with",
        ),
        pytest.param(
            ["--include-dependencies", "--start-after=qemu", "run-riscv64-purecap"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            ["extra-cmake-modules-amd64", "qtdeclarative-native", "qttools-native", "kcoreaddons-amd64"],
...
def test_kernconf():
    ...
    config = _parse_arguments(["--cheribsd-riscv64-hybrid/kernel-config=FOO BAR BAZ"])
    assert cheribsd_riscv_hybrid.kernel_config == ["FOO", "BAR", "BAZ"]
    assert cheribsd_riscv.kernel_config == ["QEMU"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # missing CHERI(-PURECAP)-NOCAPREVOKE-QEMU-MFS-ROOT
        pytest.param(
            "cheribsd-mfs-root-kernel-riscv64-purecap",
